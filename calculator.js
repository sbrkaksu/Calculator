...
keyButton.forEach((button) => {
  button.addEventListener("click", () => {
    const buttonValue = button.getAttribute("data-value");
        switch(buttonValue){
          case '.':
            if(!isScreenEmpty){                   // Do not insert '.' when the screen is empty
          ...
          case '*':
          case '/':
            if(!isScreenEmpty){                                           // Do not do anything if the the screen is empty
              numberOfOperatorClicks++;                                   // Increase the counter when user clicks '+','_','*' or '/'  
            if( numberOfOperatorClicks == 1 ){                            // When clicked for the first time
            ...
            else{                                                         // Update the operator if the user changes it
            ...
            if((buttonValue == '*' || buttonValue == '/') && (finalMem.slice(0,-1).includes("+") || finalMem.slice(0,-1).includes("-"))){   
            ...
            result.length > 13 ? (display.innerHTML = Number(result).toExponential(8)) : (display.innerHTML = Number(result));  // If the result overflows from the screen, show the result as exp
            };
             break;
            case '%':
              if(!isScreenEmpty){                                           // Do not do anything if the the screen is empty
              ...
              if( numberOfOperatorClicks == 1 ){                            
                ...
                (finalMem.endsWith('+') || finalMem.endsWith('-') || finalMem.endsWith('*') || finalMem.endsWith('/')) ?   
                finalMem = `${finalMem.slice(0,-1)}/100`:                   // if user clicks '%' after '+', '-', '*' '/', update correctly, if not continue
                finalMem = `${finalMem}/100`;                               
              }
              if(finalMem.slice(0,-4).includes("+") || finalMem.slice(0,-4).includes("-")){  // if finalMem includes  '+', '-', '*' '/', show temprorarily the last number and continue 
                if( numberOfOperatorClicks == 1 ){
              ...
              result.length > 13 ? (display.innerHTML = Number(result).toExponential(8)) : (display.innerHTML = Number(result));
              };
              break;
              case '=':
                if(!isScreenEmpty){ 
